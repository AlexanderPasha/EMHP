package com.mannydev.exmohelper;
...
import com.mannydev.exmohelper.model.Exmo;
import com.mannydev.exmohelper.model.Token;

import com.baoyz.swipemenulistview.SwipeMenuListView;
import com.google.android.gms.ads.AdView;
import com.mannydev.exmohelper.view.MyTokensListViewAdapter;
...
public class BagActivity extends AppCompatActivity {
    ...
    private void bagRefresh() {
        ...
        txtTotalBtcBalance.setText(roundResult(usdBalance / Double.parseDouble(exmo.getBTCUSD().getAvg())));
    ...
    private void addUsdToBalance(Token token) {
        ...
        if (token.getName().equals("BTC")) {
            usd = Double.parseDouble(exmo.getBTCUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("LTC")) {
            usd = Double.parseDouble(exmo.getLTCUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("ETH")) {
            usd = Double.parseDouble(exmo.getETHUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("XRP")) {
            usd = Double.parseDouble(exmo.getXRPUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("ETC")) {
            usd = Double.parseDouble(exmo.getETCUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("DASH")) {
            usd = Double.parseDouble(exmo.getDASHUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("DOGE")) {
            usd = Double.parseDouble(exmo.getDOGEBTC().getAvg())*Double.parseDouble(exmo.getBTCUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("BCH")) {
            usd = Double.parseDouble(exmo.getBCHUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("ZEC")) {
            usd = Double.parseDouble(exmo.getZECUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("XMR")) {
            usd = Double.parseDouble(exmo.getXMRUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("USDT")) {
            usd = Double.parseDouble(exmo.getUSDTUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("KICK")) {
            usd = Double.parseDouble(exmo.getKICKBTC().getAvg())*Double.parseDouble(exmo.getBTCUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("WAVES")) {
            usd = Double.parseDouble(exmo.getWAVESBTC().getAvg()) * Double.parseDouble(exmo.getBTCUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("HBZ")) {
            usd = Double.parseDouble(exmo.getHBZUSD().getAvg()) * token.getBallance();
        }
        if (token.getName().equals("EOS")) {
            usd = Double.parseDouble(exmo.getEOSUSD().getAvg()) * token.getBallance();
        }
